    ...
    return getVoices()
        .then(function(voices) {
            ...
            if (!voice && lang) {
                ...
                if (voice && isRemoteVoice(voice)) voice = Object.assign({
                    autoSelect: true
                }, voice);
            }
            return voice;
...
function findVoiceByName(voices, name) {
    for (var i = 0; i < voices.length; i++)
        if (voices[i].voiceName == name) return voices[i];
    return null;
}
...
function removeCachedAuthToken(authToken) {
    return new Promise(function(fulfill) {
        brapi.identity.removeCachedAuthToken({
            token: authToken
        }, fulfill);
    })
}
...
function getAccountInfo(authToken) {
    return ajaxGet(config.serviceUrl + "/read-aloud/get-account?t=" + authToken)
        ...
        .catch(function(err) {
            if (err.xhr && err.xhr.status == 401) return removeCachedAuthToken(authToken).then(function() {
                return null
            });
            else throw err;
        })
...
function getAllFrames(tabId) {
    return new Promise(function(fulfill) {
        brapi.webNavigation.getAllFrames({
            tabId: tabId
        }, fulfill);
    })
}
...
function getFrameTexts(tabId, frameId, scripts) {
    return new Promise(function(fulfill, reject) {
        ...
        var tasks = scripts.map(function(file) {
            return executeScript.bind(null, {
                file: file,
                tabId: tabId,
                frameId: frameId
            });
