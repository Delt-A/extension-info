function SimpleSource(texts) {
    ...
    this.getUri = function() {
        var textLen = texts.reduce(function(sum, text) {
...
function TabSource() {
    var handlers = [{
        ...
        validate: function() {
            return new Promise(function(fulfill) {
                ...
                .then(function(allowed) {
                    if (!allowed) throw new Error(JSON.stringify({
                        code: "error_file_access"
                    }));
                })
        }
    }, {
        ...
        validate: function() {
            ...
            return hasPermissions(perms)
                .then(function(has) {
                    if (!has) throw new Error(JSON.stringify({
                ...
                return getAllFrames(tab.id)
                    .then(function(frames) {
                        return frames.find(function(frame) {
                            return frame.frameId && frame.parentFrameId
                        });
                    })
                    .then(function(frame) {
    ...
    this.ready = tabPromise
        .then(function(res) {
            if (!res) throw new Error(JSON.stringify({
                code: "error_page_unreadable"
            }));
            tab = res;
            handler = handlers.find(function(h) {
                return h.match(tab.url || "")
            });
            return handler.validate();
        })
        .then(function() {
            if (handler.getFrameId)
                return getAllFrames(tab.id).then(handler.getFrameId).then(function(res) {
                    frameId = res
                });
        })
        .then(waitForConnect)
        ...
        .then(extraAction(function(info) {
            if (info.requireJs) {
                var tasks = info.requireJs.map(function(file) {
                    return inject.bind(null, file)
                });
    ...
    this.getCurrentIndex = function() {
        ...
        waiting = true;
        return peer.invoke("getCurrentIndex").finally(function() {
            waiting = false
        });
    }
        ...
        return inject("js/jquery-3.1.1.min.js")
            ...
            .then(function() {
                if (handler.extraScripts) {
                    var tasks = handler.extraScripts.map(function(file) {
                        return inject.bind(null, file)
                    });
                    return inSequence(tasks);
                }
...
function Doc(source, onEnd) {
    ...
    function close() {
        return ready
            .catch(function() {})
            .then(function() {
                if (activeSpeech) activeSpeech.stop().then(function() {
                    activeSpeech = null
                });
                source.close();
            })
        ...
        return source.getTexts(currentIndex)
            ...
            .then(function(texts) {
                if (texts) {
                    ...
                    if (hasText) onEnd();
                    else onEnd(new Error(JSON.stringify({
                        code: "error_no_text"
                    })));
                }
            })
    ...
    function browserDetectLanguage(text) {
        ...
        return new Promise(function(fulfill) {
            ...
            .then(function(result) {
                if (result) {
                    var list = result.languages.filter(function(item) {
    ...
    function serverDetectLanguage(text) {
        return ajaxPost(config.serviceUrl + "/read-aloud/detect-language", {
                text: text
            }, "json")
            .then(JSON.parse)
            .then(function(list) {
    ...
    function stop() {
        return ready
            .then(function() {
                if (activeSpeech) return activeSpeech.stop().then(function() {
                    activeSpeech = null
                });
            })
    }
