(function() {
    ...
    Promise.all([
            ...
            getVoices().then(function(voices) {
                return voices.filter(isPremiumVoice)
            }),
    ...
    function findPurchase(sku, authToken) {
        return getPurchases(authToken)
            .then(function(purchases) {
                return purchases.find(function(x) {
                    return x.sku == sku
                });
            })
    }
    ...
    function monitorPendingPurchase(sku, authToken, interval) {
        ...
        return retryUntil(getPurchaseState, isComplete, interval)
            ...
            .then(function() {
                setState("pendingPurchaseSku", null)
            })
            .then(function() {
    ...
    function onBuy(item) {
        ...
        getAuthToken({
            ...
            .then(function() {
                return preparePurchase(item.sku, authToken)
            })
            .then(function() {
                return purchase(item.sku)
            })
            .then(function() {
                return setState("pendingPurchaseSku", item.sku)
            })
            .then(function() {
                monitorPendingPurchase(item.sku, authToken, 2000)
            })
            .catch(function(err) {
