if (location.href.startsWith("https://jigsaw.")) {
    ...
    var port = brapi.runtime.connect({
        name: "ReadAloudGetTextsScript"
    });
...
function ReadAloudDoc() {
    ...
    this.getTexts = function(index) {
        var tasks = [];
        for (; currentIndex < index; currentIndex++) tasks.push(function() {
            $btnNext.click()
        }, waitMillis.bind(null, 2500));
        for (; currentIndex > index; currentIndex--) tasks.push(function() {
            $btnPrev.click()
        }, waitMillis.bind(null, 2500));
...
function getTexts() {
    ...
    var texts = $paras.get().map(function(elem) {
