    ...
    function DialogWindow() {
            ...
            if (this.dialog != null) {
                if (this.dialog.state == "pause") { //pause screen
                    ...
                    if (window.Game.home.selectedLevel > 10 &&
                        ...
                        if (this.button_1.isClicked()) {
                            buyProduct('boxel_rebound_pro');
                            this.checkGameOverOptions();
                    ...
                    if (this.button_2.isClicked()) {
                        this.quit();
                    }
                } else if (this.dialog.state == "delete") { //level editor delete confirmation
                    if (this.button_1.isClicked()) {
                        this.deleteLevel(); /*this.resume();*/
                    } else if (this.button_2.isClicked()) {
                        this.removeDialogWindow(); /*this.resume();*/
                    }
                    ...
                    if (this.button_1.isClicked()) {
                        ...
                        window.Game.shareCurrentLevel();
                    }
                } else if (this.dialog.state == "downloadOptions") { //upload successful
                    if (this.button_back.isClicked()) {
                        this.removeDialogWindow();
                    } else if (this.button_1.isClicked()) {
                        window.storageManager.loadLevelFromFile();
                        this.removeDialogWindow();
                    } else if (this.button_2.isClicked()) {
                        this.openDialogWindow("playCode");
                    }
                } else if (this.dialog.state == "uploadOptions") { //upload successful
                    if (this.button_back.isClicked()) {
                        this.removeDialogWindow();
                    } else if (this.button_1.isClicked()) {
                        window.storageManager.saveLevelToFile();
                        this.removeDialogWindow();
                    } else if (this.button_2.isClicked()) {
                        window.Game.levelEditor.shareCurrentLevel();
                    }
                    ...
                    else if (this.input.getChildByName("_b").isClicked()) {
                        this.submitCode();
                    }
        ...
        this.createDialogWindow = function(state) {
            ...
            if (state == "pause") { //menu dialog
                ...
                if (level > 10 &&
                    ...
                    this.text_2.y = this.box * 8;
                    this.text_2.set({
                        font: (this.fontSize * 0.8) + "px prstart"
                    });
        ...
        this.setDefaultDialogElements = function() {
            ...
            this.text_1.set({
                x: this.width * 0.5,
                y: this.box * 2,
                font: (this.fontSize * 1.5) + "px prstart",
            ...
            this.text_2.set({
                x: this.width * 0.5,
                y: this.box * 4,
                font: this.fontSize + "px prstart",
                color: this.fontColor,
                textAlign: "center",
                lineWidth: this.box * 12
            });
        ...
        this.quit = function(forceNextLevel) {
            ...
            if (prevView == 1) {
                window.Game.home.setUI(forceNextLevel);
        ...
        this.resume = function() {
            if (window.Game.gamePaused == true) {
                window.Game.resumeGame();
                this.removeDialogWindow();
            }
        };
        ...
        this.createInputDialog = function() {
            ...
            for (var i = 0; i < this.input.children.length; i++) {
                this.input.getChildAt(i).setFontSize(1);
            }
        };
