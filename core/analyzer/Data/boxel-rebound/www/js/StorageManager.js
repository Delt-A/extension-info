(function(window) {
    ...
    function StorageManager() {
        ...
        this.saveToLocalStorage = function(index) {
            if (index == null) {
                index = 1;
                while (localStorage.getItem(this.prefix + index) !== null) {
                    index++;
                }
            }
        ...
        this.setLevelString = function(levelString) {
            this.levelString = levelString;
        };
        this.getFromLocalStorage = function(key) {
            return localStorage.getItem(key);
        };
        this.getLocalStorageLength = function() {
            return localStorage.length;
        };
        this.getLocalStorageKey = function(index) {
        ...
        this.saveLevelToPortal = function(name, levelMap) {
            if (this.checkDailyLimit('upload')) {
                ...
                var compressedMap = levelMap; // TODO - replace this declaration after v1.6.0.3 is fully released
                // var compressedMap = LZString.compressToUTF16(levelMap);
                var data = {
                    'level_id': name,
                    'level_map': compressedMap,
                    'user_id': user_id
                };
                $.ajax({
                    type: "POST",
        ...
        this.saveLevelToFile = function() {
            ...
            var blob = new Blob([this.expandLevel()], {
