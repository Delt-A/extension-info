(function(window) {
    ...
    function LevelMap() {
        ...
        this.setTile = function(col, row, type, lockCols, lockRows) {
            ...
            if (lockRows != true) {
                ...
                while (row > (rows - 1)) { //check if row > current rows
                    this.map[rows] = [];
                    for (var c = 0; c < cols; c++) {
                        this.map[rows][c] = new this.tile(0);
                    }
                    rows++;
                }
            }
            ...
            if (lockCols != true) {
                while (nCol > 0) { //check if col < 0
                    for (var r = 0; r < rows + nRowCount; r++) {
                        try {
                            if (nRow <= 0 && row < rows) this.map[r].unshift(new this.tile(0));
                        } catch (e) {
                ...
                while (col > (cols - 1)) { //check if col > current columns
                    for (var r = 0; r < rows + nRowCount; r++) {
                        try {
                            if (nRow <= 0 && row < rows) this.map[r][cols] = new this.tile(0);
                        } catch (e) {
                            if (e instanceof TypeError) {
                                success = false;
                            }
        ...
        this.getTileRow = function(y) {
            return Math.floor((y - this.y) / this.box);
        };
        this.getTileCol = function(x) {
            return Math.floor(x / this.box);
        };
        ...
        this.insertColumn = function() {
            ...
            if (col > this.getCols()) col = this.getCols();
            for (var row = 0; row < this.getRows(); row++) {
                this.map[row].splice(col, 0, new this.tile(0));
            }
        }
        ...
        this.getCols = function() {
            return this.getRows() > 0 ? this.map[0].length : 0;
        };
        this.getWidth = function() {
            return this.getCols() * this.box;
        };
        this.getHeight = function() {
            return this.getRows() * this.box;
        };
        ...
        this.alignFromCenter = function() {
            this.y = (window.Game.getHeight() / 2) - ((this.getRows() * this.box) / 2);
        };
