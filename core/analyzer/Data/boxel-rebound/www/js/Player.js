    ...
    function Player() {
        ...
        this.tick = function(delta, map) {
            ...
            if (this.playerState == 0) { //alive
                if (this.y >= window.Game.getHeight() || this.y < -(this.box16 * 8)) {
                    this.playerState = 1;
                    createjs.Sound.play("impact");
                } //kill player if off screen
                else if (window.Game.artboard.x <= -this.x) {
                    this.playerState = 1;
                    createjs.Sound.play("impact");
                } else {
                    ...
                    if (checkX == false) {
                        window.Game.artboard.x -= (delta * this.xMV) * 0.5;
                    } //if hit wall
        ...
        this.setHitBox = function(x, y, map) {
            this.hitBox = this.checkHitBox(x, y, map);
        };
        ...
        this.checkTileAction = function() {
            for (var i = 0; i < this.hitBox.length - 2; i++) { //no need to check gap checking points
                if (this.hitBox[i].type == 2 && (i == 0 || i == 1) && Math.abs(this.hitBox[i].slope) > 1) {
                    this.pauseGame = true;
                } else if (this.hitBox[i].type == 3) { //set respawn points
                    this.spawnX = this.hitBox[i].x - this.x;
                    this.spawnY = this.hitBox[i].y;
                    this.newSpawn = true;
                    window.Game.particles.addParticles(this.x, this.y, this.scaleX / 2, window.Game.theme.main_color4, 1, 25, true);
                } else if (this.hitBox[i].type == 4 && (i == 3 || i == 2) && this.gravity.isFalling()) {
                    this.playerState = 1;
                    createjs.Sound.play("impact");
                } else if (this.hitBox[i].type == 5) {} else if (this.hitBox[i].type == 6 && (i == 0 || i == 1) && this.gravity.isRising()) {
                    this.playerState = 1;
                    createjs.Sound.play("impact");
                } else if (this.hitBox[i].type == 7 && (i == 1 || i == 2)) {
                    this.playerState = 1;
                    createjs.Sound.play("impact");
                } else if (this.hitBox[i].type == 8) {
                    this.scaleX = this.scaleY = window.Game.box16;
                    window.Game.particles.addParticles(this.x, this.y, this.scaleX / 2, window.Game.theme.main_color4, 1, 25, true);
                } //grow
                else if (this.hitBox[i].type == 9) {
                    this.scaleX = this.scaleY = window.Game.box16 / 2;
                    window.Game.particles.addParticles(this.x, this.y, this.scaleX / 2, window.Game.theme.main_color4, 1, 25, true);
                } //shrink
                else if (this.hitBox[i].type == 12 && (i == 0 || i == 1) && Math.abs(this.hitBox[i].slope) > 1) {
                    this.setGravity(0.5, true);
                } //low gravity
                else if (this.hitBox[i].type == 13 && (i == 0 || i == 1) && Math.abs(this.hitBox[i].slope) > 1) {
                    this.setSpeed(1.5, true);
                } //fast block
                else if (this.hitBox[i].type == 14 && (i == 0 || i == 1) && Math.abs(this.hitBox[i].slope) > 1) {
                    this.setGravity(1.5, true);
                } //high gravity
                else if (this.hitBox[i].type == 15 && (i == 0 || i == 1) && Math.abs(this.hitBox[i].slope) > 1) {
                    this.setSpeed(0.5, true);
                } //slow block
                else if (this.hitBox[i].type == 10 && (i == 3 || i == 2) && this.gravity.isFalling()) {
                    this.bounce = true;
                } else if (this.hitBox[i].type == 11 && (i == 3 || i == 2)) {
                    this.onFinishTile = true;
                } else if (this.hitBox[2].type != 11 && this.hitBox[3].type != 11) {
                    this.onFinishTile = false;
                }
            }
        };
        ...
        this.snapYtoMap = function(y, offset, map) {
            return (map.y + (map.getTileRow(y + Math.abs(offset)) * map.box)) - offset;
        };
        this.snapXtoMap = function(x, offset, map) {
            return (map.getTileCol(x + Math.abs(offset)) * map.box) - offset;
        };
