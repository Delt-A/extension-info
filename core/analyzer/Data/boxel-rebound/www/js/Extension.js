...
function onSkuDetailsFailed(response) {
    ...
    if (reason == 'TOKEN_MISSING_ERROR') $(".subtitle").html('Store is not available. Please log in to Google Chrome and turn on sync. <a id="faq">Need help?</a>');
    else if (reason == 'INVALID_RESPONSE_ERROR') $(".subtitle").html('Store is not available in your region. <a id="faq">Need help?</a>');
...
function viewAd() {
    ...
    if (window.storageManager.getFullscreenValue() == 1) chrome.tabs.create({
        url: adLink
    });
...
function checkExtension() {
    // create a promo condition
    var showPromo = window.storageManager.hasLicense() == false && window.storageManager.skinIsActive(3) == false;
    ...
    if (chrome.extension.ViewType.TAB == "tab" &&
        ...
        //create listeners
        document.getElementById("id-audio").addEventListener("click", function() {
            toggleVolume();
        });
        document.getElementById("id-fullscreen").addEventListener("click", function() {
            toggleFullscreen();
        });
        document.getElementById("id-fastmode").addEventListener("click", function() {
            toggleFastMode();
        });
        document.getElementById("id-browse").addEventListener("click", function() {
            chrome.tabs.create({
                url: 'www/index.html?browse=true&filter=recent'
            });
        });
        document.getElementById("id-play").addEventListener("click", function() {
            ...
            if (document.getElementById("id-fullscreen").checked == true) {
                chrome.tabs.create({
                    url: 'www/index.html?fullscreen=true'
                });
            }
        });
        document.getElementById("id-social").addEventListener("click", function() {
            if (showPromo == true) chrome.tabs.create({
                url: 'www/twitch-promo.html'
            });
            else chrome.tabs.create({
                url: 'https://chrome.google.com/webstore/detail/boxel-rebound/iginnfkhmmfhlkagcmpgofnjhanpmklb/reviews?hl=en-US'
            });
        });
        ...
        document.getElementById("version").addEventListener("click", function() {
            chrome.tabs.create({
                url: 'https://chrome.google.com/webstore/detail/boxel-rebound/iginnfkhmmfhlkagcmpgofnjhanpmklb?hl=en-US'
            });
        });
        ...
        $(document).ready(function() {
            ...
            if (showPromo == true) {
                $('#app-container').prepend('<p class="message">Want a free skin? Follow my brother on Twitch and <a href="twitch-promo.html">verify</a> your account.</p>');
            }
