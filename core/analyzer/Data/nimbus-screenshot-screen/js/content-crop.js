...
(function($) {
    ...
    function showLoupe(clientX, clientY) {
        ...
        if (clientX < lw && clientY < lh) positionLoupe = {
            x: wi - lw,
            y: hi - lh
        };
        if (clientX > wi - lw && clientY > hi - lh) positionLoupe = {
            x: 10,
            y: 10
        };
    ...
    function createCoords() {
        ...
        $('<button/>', {
            ...
            chrome.runtime.sendMessage({
                operation: 'download_screen_content'
            });
            destroyCrop();
        }).appendTo(ns_crop_buttons);
    ...
    function showCoords(data) {
        ...
        if ((data.height + data.y + 100) > window.innerHeight * z) {
            $('#ns_crop_button').css({
                'bottom': '0',
                'top': 'auto'
            });
        ...
        if (data.y < 50) $('#ns_crop_screenshot_size').css({
            'bottom': 'auto',
            'top': '0'
        });
        else $('#ns_crop_screenshot_size').css({
            'bottom': '100%',
            'top': 'auto'
        });
    }
    ...
    chrome.runtime.sendMessage({
        ...
        cropper = new Cropper(image, {
            ...
            ready: function() {
                ...
                chrome.runtime.sendMessage({
                    operation: 'get_crop_position'
                }, function(response) {
                    if (response.x && response.y && response.width && response.height) {
                        let z = window.core.is_chrome ? window.devicePixelRatio : 1;
                        cropper.crop().setCropBoxData({
                            left: response.x / z,
                            top: response.y / z,
                            width: response.width / z,
                            height: response.height / z
                        });
            ...
            cropstart: function() {
                let data = cropper.getData(true);
                createCoords(data);
                showLoupe(data.x + data.width, data.y + data.height);
        ...
        $nsc_crop.on('mousemove', function(e) {
            let z = window.core.is_chrome ? window.devicePixelRatio : 1;
            showLoupe(e.clientX * z, e.clientY * z)
        });
