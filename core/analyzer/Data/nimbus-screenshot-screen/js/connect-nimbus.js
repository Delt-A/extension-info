...
if (!window.nimbusConnectInjected) {
    ...
    var nimbusShare = {
        ...
        startUploadVideo: function(blob, channel, cb) {
            nimbusShare.server.user.info(function(info) {
                ...
                $.ajax({
                    ...
                    if (res.errorCode === 0) {
                        nimbusShare.screencastSave(res.body.files["video"], channel);
                        $.ambiance({
                            message: chrome.i18n.getMessage("notificationUploaded"),
                            timeout: 5
                        });
                    } else {
                        $.ambiance({
                ...
                if (msg.errorCode === 0) {
                    if (share) {
                        nimbusShare.shortUrl(msg.body.location, function(url) {
                            ...
                            nimbus_screen.viewClassRoomButton(url);
        ...
        screencastSave: function(tempname, channel) {
            const share = nimbusShare.notesIsShared();
            ...
            send: {
                ...
                screencast: function(req, cb) {
                    nimbusShare.server.send.file({
                        data: req.data,
                        name: screenshot.getFileName2(false, 'webm'),
                        type: 'video'
                    }, function(err, res) {
                        console.log(res);
                        if (err) return cb(err);
                        ...
                        const global_id = nimbusShare.notesGenerateId();
                        nimbusShare.server.send.request({
                            data: {
                                "action": "notes:update",
                                "body": {
                            ...
                            if (res.errorCode === -20) {
                                cb(new Error(chrome.i18n.getMessage("notificationReachedLimit")));
                            } else if (res.errorCode === -23) {
                                cb(new Error(chrome.i18n.getMessage("notificationWrong")));
                            } else if (res.errorCode === 0) {
                                nimbusShare.server.note.share(global_id, cb);
                            } else {
                ...
                screenshot: function(req, cb) {
                    nimbusShare.server.send.file({
                        ...
                        nimbusShare.server.send.request({
                            ...
                            if (res.errorCode === -20) {
                                cb && cb(core.customError(nimbusShare.server.send.xhr.status, chrome.i18n.getMessage("notificationReachedLimit")));
                            } else if (res.errorCode === -23) {
                                cb && cb(core.customError(nimbusShare.server.send.xhr.status, chrome.i18n.getMessage("notificationWrong")));
                            } else if (res.errorCode === 0) {
                                cb(null, {
                                    url: res.body.location
                                })
                            } else {
                                cb && cb(core.customError(nimbusShare.server.send.xhr.status, chrome.i18n.getMessage("Unknown error")));
                            }
                        });
                    });
                }
            ...
            user: {
                auth: function(login, password, cb) {
                    (login && password) && nimbusShare.sendNew('auth', {
                        ...
                        nimbusShare.send('https://nimbusweb.me/auth/api/applyAuth', {
        ...
        show: {
            ...
            info: function(cb) {
                nimbusShare.server.user.info(function(info) {
                    ...
                    let can_upload = (info.body.usage.notes.current + nimbus_screen.info.file.size) < info.body.usage.notes.max;
                    ...
                    if (progress > 90) {
                        ...
                        $usage_group.find('.nsc-aside-usage-line-colored').css({
                            background: '#ff0000'
                        });
                    }
                    // if (can_upload) {
