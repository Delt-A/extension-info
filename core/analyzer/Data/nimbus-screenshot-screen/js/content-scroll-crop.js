...
(function($) {
        ...
        cropper = new Cropper(image, {
            ...
            ready: function() {
                ...
                chrome.runtime.sendMessage({
                    operation: 'get_crop_scroll_position'
                }, function(response) {
    ...
    function createCoords() {
        ...
        $('<button/>', {
            ...
            chrome.runtime.sendMessage({
                'operation': 'generate_selected_scroll'
            });
            destroyCrop();
        ...
        $('<button/>', {
            ...
            chrome.runtime.sendMessage({
                operation: 'generate_selected_scroll_save'
            });
            destroyCrop();
        }).appendTo(ns_crop_buttons);
        $('<button/>', {
        ...
        if (window.core.is_firefox) {
            $('<button/>', {
                ...
                chrome.runtime.sendMessage({
                    operation: 'copy_to_clipboard_scroll'
                });
                destroyCrop();
            }).appendTo(ns_more_container);
        }
    ...
    function saveCropPosition(data) {
        chrome.runtime.sendMessage({
            operation: 'save_crop_scroll_position',
            value: data
        });
    }
    ...
    function showCoords(data) {
        ...
        if (data.width * z < 500) $('#ns_crop_more').css({
            'bottom': '0',
            'top': 'auto'
        });

        if (data.y * z < 50) $('#ns_crop_screenshot_size').css({
            'bottom': 'auto',
            'top': '0'
        });
