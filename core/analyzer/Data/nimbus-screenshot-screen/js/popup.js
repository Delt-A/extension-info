...
$(document).ready(function() {
        ...
        switch (this.name) {
            ...
            case 'record-start':
                ...
                if (localStorage.quickVideoCapture === 'true') {
                    chrome.runtime.sendMessage({
                        operation: 'content_automation',
                        action: 'video',
                        type: $('input[name=record-type]:checked').val(),
                        auth: localStorage.enableVideoEdit
                    });
                } else {
                    chrome.runtime.sendMessage({
                        operation: 'activate_record',
                        'key': 'start',
                        'value': $('input[name=record-type]:checked').val()
                    });
                }
                break;
            case 'record-stream-start':
                if (localStorage.streamWelcome !== "true") {
                    ...
                    chrome.runtime.sendMessage({
                        operation: 'content_popup',
                        action: 'nsc_popup_streamwelcome_open'
                    });
                } else {
                ...
                nimbusShare.server.workspaces.getAll(function(res) {
                    $('#workspaces_list').empty().append(function() {
                        ...
                        for (let i = 0, len = res.body.workspaces.length; len > i; i++) {
                            ...
                            let destruction = 'Персональный воркспейс';
                            if (workspace.countMembers > 0) destruction = workspace.countMembers + ' пользователей';

                            if (localStorage.workspaceSelect === 'false' && workspace.isDefault) {
                            ...
                            $workspaces.push(
                                $('<div class="nsc-workspace-item">').data('id', workspace.globalId).append([
                                    ...
                                    localStorage.workspaceSelect = $(this).data('id');
                                    $('.nsc-workspace-item').removeClass('select').filter(this).addClass('select');
                                }).addClass(function() {
                                    return $(this).data('id') === localStorage.workspaceSelect ? 'select' : '';
                                })
                            )
                        }
                        return $workspaces;
            ...
            case 'open-extensions':
                chrome.runtime.sendMessage({
                    operation: 'open_page',
                    'url': 'chrome://extensions/?id=bpconcjcammlapcogcnnelfmaeghhagj'
                });
                break;
            case 'open-nimbus-client':
                chrome.runtime.sendMessage({
                    operation: 'open_page',
                    'url': 'https://nimbus.everhelper.me/client/'
                });
                break;
    ...
    $("input").on('change', function() {
            switch (this.name) {
                ...
                case 'private-upload-video':
                    localStorage.privateUploadVideo = $(this).prop("checked");
                    setOption('privateUploadVideo', localStorage.privateUploadVideo);
                    break;
        ...
        .filter('[name=private-upload-video]').prop('checked', localStorage.privateUploadVideo !== 'false').end()
    ...
    chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
        ...
        if (request.operation === 'check_tab_action' && request.action === 'back_is_page') {
            ...
            if (actions.chrome) $nsc_button_main.not('[name=capture-window], [name=capture-blank], [name=nimbus-capture-desktop]').attr('disabled', 'disabled').css({
                opacity: 0.7
            });
            if (actions.fragment) $nsc_button_main.attr('disabled', 'disabled').not('[name=capture-fragment]').css({
                opacity: 0.7
            });
            if (actions.crop) $nsc_button_main.attr('disabled', 'disabled').not('[name=capture-area]').css({
                opacity: 0.7
            });
            if (actions.scroll_crop) $nsc_button_main.attr('disabled', 'disabled').not('[name=capture-scroll]').css({
                opacity: 0.7
            });
