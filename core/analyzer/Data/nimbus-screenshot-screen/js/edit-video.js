$(document).ready(function() {
    ...
    player.ontimeupdate = function(e) {
        ...
        if (player.currentTime > start && finish !== 0 && player.currentTime < Math.floor(finish)) {
            ...
            $('#nsc_video_time_line_cursor').animate({
                'left': start / ratio * step - 0.5
            }, time, 'linear', function() {
                window.setTimeout(function() {
                    $(this).stop().css({
                        'left': finish / ratio * step - 0.5
                    });
                }.bind(this), 250 - time)
            });
        } else {
            ...
            if (player.currentTime > duration) {
                ...
                player.pause();
                $('#nsc_video_time_line_cursor').stop().css({
                    'left': player.currentTime / ratio * step - 0.5
                });
            } else {
                $('#nsc_video_time_line_cursor').stop().animate({
                    'left': player.currentTime / ratio * step - 0.5
                }, 250, 'linear');
            }
        }
    ...
    $nsc_video_time_line.on('mousemove', function(e) {
        ...
        if (select_crop_finish) {
            $nsc_video_timeline_choose.css({
                right: offset.left + $nsc_video_time_line.width() - e.pageX + 'px'
            });
        ...
        if (select_crop_start || select_crop_finish) {
            ...
            $('#nsc_video_time_line_cursor').stop().css({
                'left': currentTime / ratio * step - 0.5
            });
    ...
    $nsc_discard.on('click', function() {
        ...
        if (result) {
            ...
            nimbus_screen.info.file.format = nimbus_screen.info.file.last_convert_format;
            nimbus_screen.info.file.last_video_name = nimbus_screen.info.file.last_convert_name;
            nimbus_screen.info.file.last_video_format = nimbus_screen.info.file.last_convert_format;

            nimbus_screen.info.file.patch = nimbus_screen.info.file.last_convert_path;
