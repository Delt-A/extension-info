            ...
            if (req.operation === 'set_watermark_video') {
                ...
                watermark.onload = function() {
                    ...
                    if ($('#nsc_watermark_video').length) {
                        $('#nsc_watermark_video').attr('src', req.dataUrl).css({
                            'top': y,
                            'left': x
                        }).show();
                    } else {
                        $('body').append($('<img>').attr('src', req.dataUrl).attr('id', 'nsc_watermark_video').css({
                            'position': 'fixed',
                            'top': y,
                            'left': x,
                    ...
                    if (req.timeEnableWatermark) {
                        window.setTimeout(function() {
                            $('#nsc_watermark_video').hide()
                        }, req.timeWatermark * 1000);
                    }
                };
