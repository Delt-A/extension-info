...
$(document).ready(function() {
    ...
    $("input, select, textarea").on('input change', function(e) {
            ...
            switch (this.name) {
                ...
                case 'font-watermark':
                    ...
                    $('textarea[name=text-watermark]').css({
                        'font-family': localStorage.fontWatermark
                    });
                    break;
                case 'size-watermark':
                ...
                case 'time-enable-watermark':
                    localStorage.timeEnableWatermark = checked;
                    window.core.setOption('timeEnableWatermark', localStorage.timeEnableWatermark);
                    window.core.setEvent('time-enable-watermark', localStorage.timeEnableWatermark);

                    break;
                case 'time-watermark':
                    localStorage.timeWatermark = this.value;
                    window.core.setOption('timeWatermark', localStorage.timeWatermark);
                    window.core.setEvent('time-watermark', localStorage.timeWatermark);
                    break;
                case 'video-quick-capture':
                    if (checked) {
                        nimbusShare.checkPremium(function(err, premium) {
                            ...
                            window.core.setOption('quickVideoCapture', localStorage.quickVideoCapture);
                ...
                case 'video-quick-capture-after':
                    ...
                    window.core.setOption('quickVideoCaptureType', localStorage.quickVideoCaptureType);
                    break;
                case 'video-quick-capture-before':
                    localStorage.enableVideoEdit = this.value;
                    window.core.setOption('enableVideoEdit', localStorage.enableVideoEdit);
                    break;
                case 'video-quick-capture-github':
                    if (checked) {
                        nimbusShare.checkPremium(function(err, premium) {
                            ...
                            window.core.setOption('quickVideoCaptureGithub', localStorage.quickVideoCaptureGithub);
                            $('#video_quick_capture_github').toggleClass('nsc-settings-choose-enabled', localStorage.quickVideoCaptureGithub !== 'false');
                        });
                    } else {
                        localStorage.quickVideoCaptureGithub = checked;
                        window.core.setOption('quickVideoCapture', localStorage.quickVideoCaptureGithub);
                        $('#video_quick_capture_github').toggleClass('nsc-settings-choose-enabled', localStorage.quickVideoCaptureGithub !== 'false');
                    }
                    break;
            }
        ...
        .filter('[name=enable-watermark]').prop('checked', localStorage.enableWatermark !== 'false').trigger('change').end()
        .filter('[name=time-enable-watermark]').prop('checked', localStorage.timeEnableWatermark !== 'false').end()
        .filter('[name=time-watermark]').val(localStorage.timeWatermark).end()
    ...
    $('#nsc_form_login_nimbus').on("submit", function() {
        ...
        if (password.value.length < 8) {
            ...
            $.ambiance({
                message: chrome.i18n.getMessage("tooltipPassInfo"),
                type: "error",
                timeout: 5
            });
            wrong = true;
        ...
        if (!/\S+@\S+\.\S+/.test(email.value)) {
            ...
            $.ambiance({
                message: chrome.i18n.getMessage("tooltipWrongEmail"),
                type: "error",
                timeout: 5
            });
            wrong = true;
    ...
    $('#nsc_form_register_nimbus').bind("submit", function() {
        ...
        if (password.value !== password_repeat.value) {
            ...
            $.ambiance({
                message: chrome.i18n.getMessage("tooltipPassMatch"),
                type: "error",
                timeout: 5
            });
            wrong = true;
        ...
        if (!wrong) {
            nimbusShare.server.user.register(email.value, password.value, function(res) {
                if (res.errorCode === 0) {
                    ...
                    $.ambiance({
                        message: chrome.i18n.getMessage("notificationEmailFail"),
                        type: "error",
                        timeout: 5
                    });
                } else {
    ...
    $('#shortcut_load_to_ns').on('change', function() {
        ...
        chrome.runtime.sendMessage({
            'operation': 'shortcut_load_to_ns_change'
        });
    });
