...
function initOpenEmailEventListeners() {
    ...
    $("#markAsRead, #markAsUnread").click(function() {
        ...
        if ($(this).attr("id") == "markAsRead") {
            ...
            showUndo({
                mail: mail,
                text: getMessage("markedAsRead"),
                undoAction: "markAsUnread"
            }).then(function() {
                openEmail({
                    mail: mail
                });
    ...
    $("#addToGoogleCalendar").click(function() {
        ...
        sendMessageToCalendarExtension({
            action: "generateActionLink",
            eventEntry: JSON.stringify(newEvent)
    ...
    $("#moveLabel").click(function() {
        ...
        mail.account.getLabels().then(async response => {
            ...
            if (await storage.get("accountAddingMethod") == "oauth") {
                ...
                labels.push({
                    id: GmailAPI.labels.CATEGORY_PERSONAL,
                    name: getMessage("primary")
                });
                labels.push({
                    id: GmailAPI.labels.CATEGORY_SOCIAL,
                    name: getMessage("social")
                });
                labels.push({
                    id: GmailAPI.labels.CATEGORY_PROMOTIONS,
                    name: getMessage("promotions")
                });
                labels.push({
                    id: GmailAPI.labels.CATEGORY_UPDATES,
                    name: getMessage("updates")
                });
                labels.push({
                    id: GmailAPI.labels.CATEGORY_FORUMS,
...
function openEmailPromise(params) {
    return new Promise(function(resolve, reject) {
        ...
        mail.getThread({
            forceDisplayImages: mail.forceDisplayImages
        }).then(async response => {
                            ...
                            if (await storage.get("replyingMarksAsRead")) {
                                if ($mail.hasClass("unread")) {
                                    ...
                                    executeAction({
                                        mail: mail,
                                        action: "markAsRead",
                                        keepInInboxAsRead: keepInInboxAsRead,
...
function openComposeSection(params) {
    ...
    $fetchContacts.off().click(function() {
        chrome.permissions.request({
            origins: [Origins.CONTACT_PHOTOS]
        }, granted => {
            if (granted) {
                oAuthForContacts.openPermissionWindow({
                        ...
                        $account.find(".setAccountIcon")
                            .click(async function() {
                                ...
                                if (await donationClicked("setAccountIcon")) {
                                    ...
                                    oAuthForProfiles.openPermissionWindow({
                                        email: account.getEmail()
                                    });
                                }
                            });
...
function renderMails(params) {
    ...
    mails.some(function(mail, mailIndex) {
        ...
        mail.getLastMessageText({
            maxSummaryLetters: maxSummaryLetters,
            htmlToText: true,
            targetNode: $mail.find(".snippet"),
    ...
    Controller.getSkins().then(async skins => {
        ...
        $dialog.find(".customSkin").off().on("click", async function() {
            ...
            $dialog.find(".updateSkin").off().on("click", async function() {
                ...
                addSkin({
                    id: "customSkin",
                    css: $dialog.find("textarea").val(),
                    image: $dialog.find("#customBackgroundImageUrl").val()
                });
                if (!await storage.get("donationClicked")) {
        ...
        if (isCtrlPressed(e)) {
            ...
            if (currentAccount) {
                ...
                if (tabletViewUrl) {
                    ...
                    if (messageId) {
                        currentAccount.openMessageById({
                            messageId: messageId
                        });
