        ...
        if (DetectClient.isChrome()) {
            fetchJSON("https://omahaproxy.appspot.com/all.json").then(data => {
                ...
                if (data.length && !versionDetected) {
                    resolve({
                        channel: "alternative based browser"
                    });
                } else {
                    resolve(versionDetected);
                }
    ...
    $nodes.each(async (index, element) => {
        ...
        $element.on(eventName, function(event) {
            if (key || params.key) {
                ...
                if (key) {
                    storagePromise = setStorage($element, {
                        event: event,
                        key: key,
                        value: value
                    });
...
function rotate(node, params) {
    ...
    if (params && params.forever) {
        node.css({
            WebkitTransition: "all 10ms linear"
        });
        var degree = 0;
        rotationInterval = setInterval(function() {
            node.css({
                WebkitTransform: 'rotate(' + (degree += 2) + 'deg)'
            }); //scale(0.4) translateZ(0)
        }, 2);
    } else {
        node.css({
...
function HttpBatch() {
    ...
    this.execute = async function(sendOAuthParams) {
        if (httpRequests.length) {
            ...
            if (!tokenResponse) {
                tokenResponse = await sendOAuthParams.oauthRequest.findTokenResponse({
                    userEmail: sendOAuthParams.email
                });
            }
...
function OAuthForDevices(defaultParams) {
    ...
    this.generateURL = async function(userEmail, url) {
        const tokenResponse = await that.findTokenResponse({
            userEmail: userEmail
        });
        if (tokenResponse) {
    ...
    async function refreshToken(tokenResponse) {
        ...
        try {
            data = await oauthFetch(GOOGLE_TOKEN_URL, data, {
                method: "post"
            });
        } catch (error) {
            error.tokenResponse = tokenResponse;
    ...
    this.ensureTokenForEmail = async function(userEmails) {
        ...
        await asyncForEach(userEmails, async (userEmail, index) => {
            var tokenResponse = await that.findTokenResponse({
                userEmail: userEmail
            });
            if (tokenResponse) {
            ...
            if (!paused) {
                if (isSyncable(params.key)) {
                    ...
                    if (localStorage.lastSyncOptionsLoad || await storage.get("lastSyncOptionsSave") || storageEventsCount >= MIN_STORAGE_EVENTS_COUNT_BEFORE_SAVING) {
                        ...
                        chrome.alarms.create(Alarms.SYNC_DATA, {
                            delayInMinutes: 1
                        });
                    } else {
                        ...
                        chrome.storage.sync.clear(function() {
                            if (chrome.runtime.lastError) {
                                ...
                                try {
                                    ...
                                    var details = {
                                        chunkId: chunkId,
                                        localStorageChunksCount: localStorageChunks.length,
                                        indexedDBChunksCount: indexedDBChunks.length,
                        ...
                        if (chrome.runtime.lastError) {
                            ...
                            if (isEmptyObject(items)) {
                                ...
                                if (details.extensionVersion != chrome.runtime.getManifest().version) {
                                    reject({
                                        items: items,
                                        error: "Versions are different: " + details.extensionVersion + " and " + chrome.runtime.getManifest().version
                                    });
                                } else {
                                    resolve(items);
                                }
...
function generateBlob(data, mimeType) {
    var blob = new Blob([convertBase64ToBlob(data)], {
        type: mimeType
    });
    return blob;
}
    ...
    DateTimeHighlighter.init = function() {
        ...
        for (var dayNameIndex = 0; dayNameIndex < dateFormat.i18nEnglish.dayNames.length; dayNameIndex++) {
            ...
            for (var monthNameIndex = 0; monthNameIndex < dateFormat.i18nEnglish.monthNames.length; monthNameIndex++) {
                ...
                myDateRegexs.push({
                    pattern: dayNamePattern + SEP + monthNamePattern + SEP + dateYearPattern + SEP + timePattern + TO + timePattern,
                    startTimeOffset: 5,
                    endTimeOffset: 11,
                    month: monthNameIndex,
                    date: function(pieces, month) {
                        var date = new Date();
                        date.setMonth(month);
                ...
                myDateRegexs.push({
                    pattern: dayNamePattern + SEP + monthNamePattern + SEP + dateYearPattern + SEP + timePattern,
                    startTimeOffset: 5,
                    month: monthNameIndex,
                    date: function(pieces, month) {
                        var date = new Date();
                ...
                myDateRegexs.push({
                    pattern: dayNamePattern + SEP + monthNamePattern + SEP + datePattern + SEP + timePattern,
                    startTimeOffset: 3,
                    month: monthNameIndex,
                    date: function(pieces, month) {
                        var date = new Date();
                ...
                myDateRegexs.push({
                    pattern: dayNamePattern + SEP + monthNamePattern + SEP + datePattern,
                    month: monthNameIndex,
                    date: function(pieces, month) {
                        var date = new Date();
                        date.setMonth(month);
                ...
                myDateRegexs.push({
                    pattern: dayNamePattern + SEP + datePattern + SEP + monthNamePattern,
                    month: monthNameIndex,
                    date: function(pieces, month) {
                        var date = new Date();
                        date.setMonth(month);
            ...
            addDayTimeRegex(myDateRegexs, {
                pattern: dayNamePattern + SEP + timePattern + TO + timePattern,
                date: date,
                startTimeOffset: 1,
                endTimeOffset: 7,
                allDay: false
            ...
            // day + time (friday 10)
            addDayTimeRegex(myDateRegexs, {
                pattern: dayNamePattern + SEP + timePattern,
                date: date,
                startTimeOffset: 1,
                allDay: false
            });
            ...
            addDayTimeRegex(myDateRegexs, {
                pattern: timePattern + SEP + dayNamePattern,
                date: date,
                startTimeOffset: 1,
                allDay: false
            });
        ...
        for (var monthNameIndex = 0; monthNameIndex < dateFormat.i18nEnglish.monthNames.length; monthNameIndex++) {
            ...
            myDateRegexs.push({
                pattern: monthNamePattern + SEP + dateYearPattern + SEP + timePattern + TO + timePattern,
                startTimeOffset: 5,
                endTimeOffset: 11,
                month: monthNameIndex,
                date: function(pieces, month) {
            ...
            // April 8, 2012, 4:00pm 
            myDateRegexs.push({
                pattern: monthNamePattern + SEP + dateYearPattern + SEP + timePattern,
                startTimeOffset: 5,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
            ...
            // 8 April 2012, 4:00pm 
            myDateRegexs.push({
                pattern: datePattern + SEP + monthNamePattern + SEP + yearPattern + SEP + timePattern,
                startTimeOffset: 4,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
            ...
            // April 8, 2012
            myDateRegexs.push({
                pattern: monthNamePattern + SEP + dateYearPattern,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
                    date.setMonth(month);
            ...
            myDateRegexs.push({
                pattern: datePattern + SEP + monthNamePattern + SEP + yearPattern,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
                    date.setMonth(month);
            ...
            // April 8, 4:00pm 
            myDateRegexs.push({
                pattern: monthNamePattern + SEP + datePattern + SEP + timePattern,
                startTimeOffset: 3,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
            ...
            // April 22 
            myDateRegexs.push({
                pattern: monthNamePattern + SEP + datePattern,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
                    date.setMonth(month);
            ...
            // 20 - 22 April
            myDateRegexs.push({
                pattern: datePattern + TO + datePattern + SEP + monthNamePattern,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
                    date.setMonth(month);
            ...
            // 22 April
            myDateRegexs.push({
                pattern: datePattern + SEP + monthNamePattern,
                month: monthNameIndex,
                date: function(pieces, month) {
                    var date = new Date();
                    date.setMonth(month);
        ...
        myDateRegexs.push({
            pattern: tomorrowPattern + SEP + timePattern + TO + timePattern,
            startTimeOffset: 1,
            endTimeOffset: 7,
            date: tomorrow(),
            allDay: false
        ...
        myDateRegexs.push({
            pattern: tomorrowPattern + SEP + timePattern,
            startTimeOffset: 1,
            date: tomorrow(),
            allDay: false
        });
        ...
        myDateRegexs.push({
            pattern: timePattern + SEP + tomorrowPattern,
            startTimeOffset: 1,
            date: tomorrow(),
            allDay: false
        });
        myDateRegexs.push({
            pattern: tomorrowPattern,
            date: tomorrow(),
            allDay: true
        });

        myDateRegexs.push({
            pattern: timePattern + TO + timePatternSolo,
            startTimeOffset: 1,
            endTimeOffset: 7,
            date: today(),
            allDay: false
    ...
    return new Promise((resolve, reject) => {
        function createObjectStore(db) {
            return new Promise((resolve, reject) => {
                ...
                var objectStore = db.createObjectStore(storeId, {
                    keyPath: "key"
                }); // Create unique identifier for store
                console.log("objectStore", objectStore)
    ...
    return new Promise((resolve, reject) => {
        ...
        function checkIfAllComplete() {
            if (successfulPromises.length + failedPromises.length == promises.length) {
                resolve({
                    successful: successfulPromises,
                    failures: failedPromises
                });
            }
        }
