...
function onLoad() {
    if (location.href.indexOf("mui=checkerPlusForGmail") != -1) {
        ...
        function syncCurrentEmail() {
            popupWindowPort.postMessage({
                action: "getCurrentEmail",
                email: getCookie("GAUSR")
            });
        }
        ...
        popupWindowPort = chrome.runtime.connect({
            name: "popupWindowAndTabletFrameChannel"
        });
