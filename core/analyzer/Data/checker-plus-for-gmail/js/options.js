    ...
    if (!$("#" + tabName + "Page").length) {
        ...
        if (tabName == "welcome") {
            ...
            $("#testOutPopupWindow").click(function() {
                openTabOrPopup({
                    url: "https://mail.google.com?view=cm&fs=1&tf=1",
                    name: "test",
                    testingOnly: true
                });
            });
            ...
            function reinitContextMenu() {
                ...
                window.initQuickContactContextMenuTimeout = setTimeout(function() {
                    ...
                    sendMessageToBG("initQuickContactContextMenu", {
                        update: true
                    });
                }, 200);
            ...
            $("#accountAddingMethod paper-radio-button").click(async () => {
                ...
                await alwaysPromise(pollAndLoad({
                    showNotification: false,
                    refresh: true
                }));
            ...
            $("#addAccount").click(async function() {
                ...
                if ((tokenResponses && tokenResponses.length) || DetectClient.isFirefox()) {
                    requestPermission({
                        oAuthForDevices: oAuthForEmails,
            ...
            $("#exportIndexedDB").click(function() {
                bg.syncOptions.exportIndexedDB({
                    exportAll: true
                }, function(response) {
                    if (response.error) {
            ...
            $("#console_messages").change(function() {
                openGenericDialog({
                    content: this.checked ? "logging enabled" : "logging disabled",
...
async function generateSoundOptions(account, labelValue) {
    ...
    if (template) {
        ...
        addPaperItem({
            node: paperMenuDiv,
            value: "custom",
            label: getMessage("uploadSound"),
            icon: "cloud-upload"
        });
        addPaperItem({
            node: paperMenuDiv,
            value: "record",
            label: getMessage("recordSound"),
            icon: "av:mic"
        });
                            ...
                            navigator.mediaDevices.getUserMedia({
                                ...
                                mediaRecorder.ondataavailable = function(e) {
                                    ...
                                    blob = new Blob(chunks, {
                                        'type': 'audio/webm'
                                    }); //  'audio/webm'  OR   audio/ogg; codecs=opus
                    ...
                    $dialog.find(".okDialog").off().click(function() {
                        if ($("#recordedSoundTitle")[0].validate()) {
                            addCustomSound({
                                ...
                                account: account,
                                labelValue: labelValue,
                                title: $("#recordedSoundTitle").val(),
                                data: $dialog.find("source")[0].src,
                                overwrite: true
                            });
                            $dialog[0].close();
                            showMessage(getMessage("done"));
        ...
        if (duplicateWarning) {
            ...
            if (await account.hasHiddenTabs()) {
                ...
                openDialog($dialog).then(response => {
                    if (response == "ok") {
                        chrome.tabs.create({
                            url: "https://jasonsavard.com/wiki/Gmail_tabs?ref=primaryLabelChecked"
                        });
                    }
                });
            }
        ...
        chrome.tts.getVoices(function(voices) {
            ...
            for (var i = 0; i < voices.length; i++) {
                if (voices[i].voiceName == "native") {
                    ...
                    var optionsObj = {
                        label: voices[i].voiceName,
                        value: voices[i].voiceName
                    };
...
async function addCustomSound(params) {
    ...
    if (params.overwrite && existingCustomSoundIndex != -1) {
        customSounds[existingCustomSoundIndex] = {
            name: title,
            data: params.data
        };
    } else {
...
async function grantPermissionToEmails(tokenResponse) {
    ...
    account.fetchSendAs().catch(error => {
        ...
        account.getEmails().then(() => {
            loadAccountsOptions({
                selectedEmail: account.getEmail()
            });
            hideLoading();
...
async function initPopupWindowOptions(value) {
    ...
    if (value == BROWSER_BUTTON_ACTION_CHECKER_PLUS || value == BROWSER_BUTTON_ACTION_CHECKER_PLUS_POPOUT) {
        ...
        initOnlyWithCheckerPlusPopupWarning({
            remove: true
        })
            ...
            if (chrome.runtime.requestUpdateCheck) {
                chrome.runtime.requestUpdateCheck(function(status, details) {
                    ...
                    if (status == "no_update") {
                        ...
                        openGenericDialog({
                            title: "Throttled, try again later!"
                        });
                    } else {
                        openGenericDialog({
                            title: "Response: " + status + " new version " + details.version
                        });
                    }
                });
            } else {
