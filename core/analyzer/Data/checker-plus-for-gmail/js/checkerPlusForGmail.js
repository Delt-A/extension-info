...
async function updateBadge(totalUnread) {
    ...
    if (accountsSummary.signedIntoAccounts == 0) {
        ...
        if (accountsSummary.firstNiceError) {
            chrome.browserAction.setTitle({
                title: accountsSummary.firstNiceError.toString()
            });
        }
        ...
        if (!totalUnread || totalUnread <= 0) {
            ...
            if (dndState) {
                ...
                chrome.browserAction.setBadgeBackgroundColor({
                    color: await storage.get("unreadCountBackgroundColor")
                });
                ...
                if (await storage.get("showButtonTooltip")) {
                    ...
                    if (mails) {
                        mails.forEach(function(mail, mailIndex) {
                            str += mail.authorName.getFirstName() + ": " + mail.title + " - " + mail.getLastMessageText({
                                maxSummaryLetters: 20,
                                htmlToText: true,
    ...
    try {
        chrome.contextMenus.update(ContextMenu.MARK_ALL_AS_READ, {
            visible: (totalUnread >= 1)
        });
    } catch (error) {
...
async function fetchContacts(userEmail) {
    ...
    return {
        ...
        contactsHaveBeenUpdated: contactsHaveBeenUpdated
    };
}
...
async function getContacts(params, callback) {
    ...
    if (contactsData) {
        ...
        if (params.account) {
            ...
            if (dataIndex != -1) {
                callback({
                    contacts: contactsData[dataIndex].contacts
                });
            ...
            callback({
                error: "No account passed"
            });
            console.log("not found")
        }
    } else {
        callback({
            error: "No cache created yet for contactsData"
        });
...
async function generateComposeUrl(params) {
    ...
    if (await storage.get("useBasicHTMLView")) {
        url = params.account.getMailUrl({
            urlParams: "v=b&pv=tl&cs=b&f=1"
        });
    } else {
        // using /u/1 etc. defaults to /u/0 always, so must use authuser
        url = params.account.getMailUrl({
            urlParams: "view=cm&fs=1&tf=1&authuser=" + encodeURIComponent(params.account.getEmail())
        });
    }
...
async function ensureUserHasUnreadEmails() {
    ...
    return {
        hasUnreadEmails: lsNumber("unreadCount")
    };
}

function insertSpeechRecognition(tabId) {
    chrome.tabs.insertCSS(tabId, {
        file: "/css/speechRecognition.css"
    }, function() {
...
function fixRelativeLinks(node, mail) {
    node.querySelectorAll("a, img, imghidden").forEach(elem => {
        ...
        if (/^\/\//.test(href)) { // starts with //
            ...
            elem.setAttribute("href", mail.account.getMailUrl({
                useStandardGmailUrl: true
            }) + href);
        } else if (/^\/|^\?/.test(src)) { // starts with / or ?
            elem.setAttribute("src", mail.account.getMailUrl({
                useStandardGmailUrl: true
            }) + src);
        }
    });
    ...
    this.setIcon = async function(params) {
        ...
        if (iconSet == "custom" && customButtonIconCanvas) {
            ...
            if (iconSet == "default") {
                ...
                chrome.browserAction.setIcon({
                    path: await self.generateButtonIconPath(params)
                });
            }
        }
...
async function initQuickContactContextMenu(params = {}) {
    ...
    if (await storage.get("showContextMenuItem")) {
        // Send page link
        createContextMenu(ContextMenu.SEND_PAGE_LINK, getMessage("sendPageLinkTitle") + "...", {
            contexts: ContextMenu.AllContextsExceptBrowserAction
        });

        if (quickComposeEmail) {
            // Send page link to [user]
            createContextMenu(ContextMenu.SEND_PAGE_LINK_TO_CONTACT, emailPageLinkToContactLabel, {
                contexts: ContextMenu.AllContextsExceptBrowserAction
            });

            if (await storage.get("accountAddingMethod") == "oauth") {
                // Send page link to [user] with message
                createContextMenu(ContextMenu.SEND_PAGE_LINK_TO_CONTACT_WITH_MESSAGE, emailPageLinkToContactWithMessageLabel, {
                    contexts: ContextMenu.AllContextsExceptBrowserAction
                });
            }
        }
...
function requestPermission(params = {}) {
    return new Promise((resolve, reject) => {
        ...
        if (params.useGoogleAccountsSignIn) {
            ...
            // Chrome sign-in
            params.oAuthForDevices.getAccessToken({
                refetch: true
            }).then(tokenResponse => {
    ...
    oAuthForProfiles = new OAuthForDevices({
        ...
        getUserEmail: function(tokenResponse, sendOAuthRequest) {
            return Promise.resolve({
                userEmail: ls["emailAccountRequestingOauth"]
            });
        }
    });
