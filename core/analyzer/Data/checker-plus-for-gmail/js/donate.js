    ...
    if (DetectClient.isFirefox()) {
        ...
        $('#video').on("load", function() {
            $(this).contents().find("body").off().on('click', () => {
                chrome.tabs.create({
                    url: "https://www.youtube.com/watch?v=Ue-li7gl3LM"
                });
            });
        });
    } else {
...
async function createStripeSource(params) {
    ...
    */
    ...
    data = await paymentFetch("https://apps.jasonsavard.com/paymentSystems/stripe/ajax.php", data, {
        method: "POST"
    });
...
async function initPaymentProcessor(price) {
    if (donateButtonClicked == "paypal") {
        ...
        location.href = await paymentFetch(Controller.FULLPATH_TO_PAYMENT_FOLDERS + "paymentSystems/paypal/createPayment.php", data, {
            method: "POST"
        });
        ...
        showLoading();
        data = await paymentFetch("https://apps.jasonsavard.com/paymentSystems/coinbase/ajax.php", data, {
            method: "post"
        });
    ...
    if (DetectClient.isFirefox()) {
        ...
        $('#video').on("load", function() {
            $(this).contents().find("body").on('click', () => {
                chrome.tabs.create({
                    url: "https://www.youtube.com/watch?v=fKNZRkhC3OE"
                });
            });
        });
    } else {
    ...
    (async () => {
        ...
        */
        ...
        $("#driveExtraFeatures").click(function() {
            chrome.tabs.create({
                url: "https://jasonsavard.com/Checker-Plus-for-Google-Drive?ref=contributePage"
            });
        });

        $("#gmailExtraFeatures").click(function() {
            chrome.tabs.create({
                url: "https://jasonsavard.com/Checker-Plus-for-Gmail?ref=contributePage"
            });
        });

        $("#calendarExtraFeatures").click(function() {
            chrome.tabs.create({
                url: "https://jasonsavard.com/Checker-Plus-for-Google-Calendar?ref=contributePage"
            });
        });

        $("#support").click(function() {
            chrome.tabs.create({
                url: "https://jasonsavard.com/forum/?ref=contributePage"
            });
        });
