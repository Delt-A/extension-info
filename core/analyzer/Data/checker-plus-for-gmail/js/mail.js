...
class Mail {
    constructor() {
        ...
        this.queueFile = function(messageId, file) {
            ...
            queuedFile.fetchPromise = that.account.fetchAttachment({
                messageId: messageId,
        ...
        this.getDate = function(twentyFourHourMode) {
            return that.issued.displayDate({
                relativeDays: true,
        ...
        this.getUrl = function() {
            return that.account.getMailUrl({
                mail: that,
                useGmailUI: true
            });
        };
        ...
        this.markAsUnread = function() {
            return that.account.executeMailAction({
                mail: that,
                action: MailAction.MARK_AS_UNREAD
            });
        };
        ...
        this.removeLabel = function(label) {
            ...
            return that.account.executeMailAction({
                mail: that,
                action: MailAction.REMOVE_LABEL,
                label: label
            });
        };
        ...
        this.reply = async function() {
            var replyObject = await that.generateReplyObject({
                type: "text"
            });
        ...
        this.getThread = async function(params = {}) {
            ...
            if (params.mail.messages || that.account.getAccountAddingMethod() == "oauth") {
                ...
                let data;
                try {
                    data = await fetchText(mail.account.getMailUrl({
                        useStandardGmailUrl: true,
                        urlParams: "ui=2&view=pt&search=all&th=" + mail.id
                    }))
                } catch (error) {
                    throw error;
                }
