...
function allowAllUrls(url, mime) {
    return url;
}

function rewriteIds(id) {
    return HTML_CSS_SANITIZER_REWRITE_IDS_PREFIX + id;
}
...
chrome.runtime.onInstalled.addListener(async details => {
    ...
    if (details.reason == "install" && !await storage.get("installDate")) {
        ...
        if (realUpdate) {
            ...
            chrome.alarms.create(Alarms.EXTENSION_UPDATED_SYNC, {
                delayInMinutes: 60
            });
        }
                    ...
                    if (account) {
                        if (!account.getHistoryId() || message.data.historyId > account.getHistoryId()) {
                            ...
                            if (lastGmailAPIActionByExtension.diffInSeconds() >= -MIN_SECONDS_BETWEEN_MODIFICATIONS_BY_EXTENSION_AND_GCM_MESSAGES) { // avoid race condition
                                chrome.alarms.create(Alarms.GET_EMAILS_FROM_FCM_UPDATE_PREFIX + email, {
                                    delayInMinutes: 1
                                });
                            } else {
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
    ...
    if (ContextMenu.OPEN_GMAIL == info.menuItemId) {
        ...
        if (await storage.get("accountAddingMethod") == "autoDetect") {
            ...
            }; // name:storage.get("quickComposeEmailAlias")
...
function ensureDoNotShowNotificationIfGmailTabOpenTest(params = {}) {
    return new Promise(async (resolve, reject) => {
        if (params.testType) {
            ...
            chrome.tabs.query({
                url: accountWithNewestMail.getMailUrl() + "*"
            }, function(tabs) {
            ...
            if (passedDoNotShowNotificationIfGmailTabOpenTest) {
                ...
                if (textNotification || !chrome.notifications) {
                    ...
                    if (notificationFlagForLabelsOfNewestEmail || params.testType) {
                        ...
                        var body = firstEmail.getLastMessageText({
                            maxSummaryLetters: 101,
                            htmlToText: true
                        });
                    ...
                    if (params.emails.length == 1) {
                        ...
                        if (notificationFlagForLabelsOfNewestEmail || params.testType) {
                            ...
                            if (notificationDisplay == "newEmail") {
                                ...
                                let emailMessage = firstEmail.getLastMessageText({
                                    maxSummaryLetters: 170,
                                    htmlToText: true,
                                    EOM_Message: " [" + getMessage("EOM") + "]"
                                });
                                if (!emailMessage) {
                                    emailMessage = "";
                                }
...
async function checkEmails(source) {
    ...
    checkingEmails = true;
    const allEmailsCallbackParams = await getAllEmails({
        accounts: accounts
    });
    await mailUpdate({
        showNotification: true,
...
function executeAction(mails, actionName) {
    if (mails.length <= MAX_EMAILS_TO_ACTION) {
        ...
        mails.forEach(mail => {
            var promise = mail[actionName]({
                instantlyUpdatedCount: true
            });
    ...
    if (changeInfo.status == "loading") {
        ...
        if (tab.url) {
            ...
            if (tab.url.indexOf(MAIL_DOMAIN + MAIL_PATH) == 0 && !alreadyDetectedInbox) {
                ...
                pollAccounts({
                    noEllipsis: true,
                    forceResyncAccounts: true,
                    source: Source.SIGN_IN
                });
            }
    ...
    chrome.commands.onCommand.addListener(async command => {
        ...
        if (command == "markAsReadInNotificationWindow") {
            ...
            if (await storage.get("desktopNotification") != "rich") {
                ...
                if (richNotifId) {
                    performButtonAction({
                        notificationButtonValue: "markAsRead",
                        notificationId: richNotifId
                    });
                } else {
                    errorFlag = true;
                }
...
function init() {
    ...
    chrome.browserAction.setTitle({
        title: getMessage("loadingSettings") + "..."
    });
    ...
    storagePromise.then(async () => {
        ...
        if (chrome.contextMenus) {
            ...
            createContextMenu(ContextMenu.DND_OFF, getMessage("turnOff"), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(null, null, {
                parentId: ContextMenu.DND_MENU,
                type: "separator"
            });
            createContextMenu(ContextMenu.DND_30_MIN, getMessage("Xminutes", 30), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(ContextMenu.DND_1_HOUR, getMessage("Xhour", 1), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(ContextMenu.DND_2_HOURS, getMessage("Xhours", 2), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(ContextMenu.DND_4_HOURS, getMessage("Xhours", 4), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(ContextMenu.DND_8_HOURS, getMessage("Xhours", 8), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(ContextMenu.DND_TODAY, getMessage("today"), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(ContextMenu.DND_INDEFINITELY, getMessage("indefinitely"), {
                parentId: ContextMenu.DND_MENU
            });
            createContextMenu(null, null, {
                parentId: ContextMenu.DND_MENU,
            ...
            createContextMenu(ContextMenu.DND_OPTIONS, getMessage("options") + "...", {
                parentId: ContextMenu.DND_MENU
            });

            try {
                createContextMenu(ContextMenu.MARK_ALL_AS_READ, getMessage("markAllAsRead"), {
                    visible: false
                });
            } catch (error) {
        ...
        chrome.alarms.create(Alarms.EVERY_MINUTE, {
            periodInMinutes: 1
        });
        chrome.alarms.create(Alarms.UPDATE_CONTACTS, {
            periodInMinutes: 60 * 4
        }); // 4 hours (used to be every 24 hours)
        chrome.alarms.create(Alarms.UPDATE_SKINS, {
            periodInMinutes: 60 * 24 * 1
        }); // 1 day (used to be every 2 days)
        chrome.alarms.create(Alarms.UPDATE_UNINSTALL_URL, {
            periodInMinutes: 60 * 24 * 1
        }); // 1 day
        chrome.alarms.create(Alarms.SYNC_SIGN_IN_ORDER, {
            periodInMinutes: 60 * 24 * 5
        }); // 5 days
        chrome.alarms.create(Alarms.CLEAR_SUBJECTS_SPOKEN, {
        ...
        if (await daysElapsedSinceFirstInstalled() > 14 && (!lastOptionStatsSent || lastOptionStatsSent.daysInThePast() >= 7)) { // start after 2 weeks to give people time to decide and then "every" 7 days after that (to keep up with changes over time)
            ...
            chrome.alarms.create(Alarms.COLLECT_STATS, {
                delayInMinutes: 2
            });
        }
...
async function pollAccounts(params = {}) {
    if (pollingAccounts && !params.refresh) {
        ...
        try {
            ...
            chrome.browserAction.setTitle({
                title: getMessage("pollingAccounts") + "..."
            });
            ...
            if (accountsSummary.signedIntoAccounts == 0) {
                setSignedOut({
                    title: accountsSummary.firstNiceError
                });
            } else {
                ...
                if ((hearSubject || hearMessage) && !voiceMessageAttachmentObj.found) {
                    ...
                    if (hearMessage) {
                        ...
                        var messageText = newestEmail.getLastMessageText({
                            maxSummaryLetters: spokenWordsLimitLength,
                            htmlToText: true
                        });
                        if (messageText) {
            ...
            if (index + 1 <= MAX_EMAILS_TO_ACTION) {
                var promise = new Promise((resolve, reject) => {
                    setTimeout(function() {
                        var markAsPromise;
                        if (action == "archive") {
                            markAsPromise = mail.archive({
                                instantlyUpdatedCount: true
                            });
                        } else if (action == "markAsRead") {
                            markAsPromise = mail.markAsRead({
                                instantlyUpdatedCount: true
                            });
                        }
