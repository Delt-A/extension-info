...
function Account() {
    ...
    async function getFeedUnreadCount(params, rawFeed) {
        ...
        if (!feedUnreadCount) {
            ...
            if (feedUnreadCount >= MAX_EMAILS_IN_ATOM_FEED && params.monitorLabels[params.monitorLabelsIndex] == SYSTEM_ALL_MAIL) {
                ...
                try {
                    const data = await ajaxBasicHTMLGmail(that.getMailUrl({
                        useBasicGmailUrl: true
                    }) + "?s=q&q=label%3Aunread"); // append 'unread' to only fetch unreads of this label of course
    ...
    async function fetchFeed(params) {
        ...
        var url = that.getMailUrl({
            useStandardGmailUrl: true,
            atomFeed: atomFeed,
            urlParams: "timestamp=" + Date.now()
        });
    ...
    this.enablePushNotifications = async function() {
        ...
        try {
            ...
            if (watchRetries[email]++ < MAX_RETRIES) {
                ...
                chrome.alarms.create(Alarms.ENABLE_PUSH_NOTIFICATIONS_EMAIL_ALARM_PREFIX + email, {
    ...
    this.watch = async () => {
        ...
        try {
            ...
            if (watchRetries[email]++ < MAX_RETRIES) {
                ...
                // minimum 1 min due to chrome alarm limitation
                const delayInMinutes = Math.max(1, exponentialRetryInSeconds / 60);
                chrome.alarms.create(Alarms.WATCH_EMAIL_ALARM_PREFIX + email, {
                    delayInMinutes: delayInMinutes
                });
            }
            await storage.set("_watchRetries", watchRetries);
    ...
    function initNewestEmails(mailObject) {
        ...
        return {
            emailAlreadyFoundInADifferentLabelFetch: emailAlreadyFoundInADifferentLabelFetch
        };
    }
    ...
    this.getEmails = async function(params = {}) {
        ...
        if (accountAddingMethod == "autoDetect") {
            const fetchEmailsResponse = await fetchEmailsByLabel({
                monitorLabels: accountMonitorLabels,
                monitorLabelsIndex: 0,
                feedsArrayInfo: []
            });
            var inboxFeed;
            ...
            async function getInitialMessages(params) {
                ...
                try {
                    const newestMailArray = await getMessagesByList({
                        monitoredLabels: accountMonitorLabels,
            ...
            try {
                ...
                if (historyId) {
                    var getMessagesByHistoryParams = {
                        historyId: historyId,
                        histories: []
                    };
                    try {
                        ...
                        if (histories && histories.length) {
                            ...
                            var createResponse = await createMailObjects({
                                httpBodies: fetchMessagesByIdsResponse.httpBodies
                            });
                ...
                return getInitialMessages({
                    account: that,
                    getEmailsResponse: getEmailsResponse
                });
            } catch (error) {
                setAccountError(that, error);
                throw error;
    ...
    async function getMessagesByList(messagesByListParams) {
        ...
        promises.push(async function() {
            ...
            if (messagesByListParams.historyId) {
                return {
                    historyId: messagesByListParams.historyId
                };
            } else {
    ...
    async function getMessages(labels, processMessages) {
        ...
        if (messagesFound) {
            ...
            await processMessages({
                fetchMessageIdsByLabelsResponse: fetchMessageIdsByLabelsResponse,
                fetchMessagesByLabelsResponse: fetchMessagesByLabelsResponse
            });
        } else {
            // should still execute processMessages
            await processMessages({
                fetchMessageIdsByLabelsResponse: fetchMessageIdsByLabelsResponse,
                labelsWithoutMessages: labels
            });
        }
    }
    ...
    async function fetchMessagesByLabels(fetchMessageIdsByLabelsResponse) {
        ...
        fetchMessageIdsByLabelsResponse.httpBodies.forEach(httpBody => {
            if (httpBody.error) {
                ...
                var fetchMessagesByIdsParams = generateFetchMessagesByIdsParams({
                    httpMessages: httpBody.messages,
                    allLabelsMessageIdsToFetch: allLabelsMessageIdsToFetch
                });
    ...
    async function createMailObjects(params) {
        ...
        return {
            ...
            totalMessagesThatWereGrouped: totalMessagesThatWereGrouped
        };
    }
    ...
    async function fetchMessageIdsByLabels(monitoredLabels) {
        ...
        */
        ...
        const batchResponse = await mygapiClient.HttpBatch.execute({
            oauthRequest: oAuthForEmails,
            email: email
        });
        ...
        return {
            httpBodies: batchResponse.httpBodies
        };
    }
    ...
    this.executeMailAction = async function(params) {
        ...
        if (accountAddingMethod == "autoDetect") {
            ...
            if (params.action == MailAction.MARK_AS_READ) {
                params.data = {
                    removeLabelIds: [GmailAPI.labels.UNREAD]
                };
                promise = executeGmailAPIAction(params);
            } else if (params.action == MailAction.MARK_AS_UNREAD) {
                params.data = {
                    addLabelIds: [GmailAPI.labels.UNREAD]
                };
                promise = executeGmailAPIAction(params);
            } else if (params.action == MailAction.DELETE) {
                ...
                params.data = {
                    addLabelIds: [GmailAPI.labels.INBOX]
                };
                return executeGmailAPIAction(params);
            } else if (params.action == MailAction.ARCHIVE) {
                params.data = {
                    removeLabelIds: [GmailAPI.labels.INBOX]
                };
                promise = executeGmailAPIAction(params);
            } else if (params.action == MailAction.MARK_AS_SPAM) {
                params.data = {
                    addLabelIds: [GmailAPI.labels.SPAM]
                };
                promise = executeGmailAPIAction(params);
            } else if (params.action == MailAction.APPLY_LABEL) {
                params.data = {
                    addLabelIds: [params.label]
                };
                ...
                params.data = {
                    addLabelIds: [GmailAPI.labels.STARRED]
                };
                await executeGmailAPIAction(params);
                ...
                params.data = {
                    removeLabelIds: [GmailAPI.labels.STARRED]
                };
                await executeGmailAPIAction(params);
                promise = Promise.resolve();
    ...
    async function fetchGmailAT1() {
        let data;
        try {
            data = await ajaxBasicHTMLGmail(that.getMailUrl({
                useBasicGmailUrl: true
            }) + Math.ceil(1000000 * Math.random()));
        } catch (error) {
    ...
    function loadMailInGmailTab(params) {
        return new Promise((resolve, reject) => {
            // focus window
            chrome.windows.update(params.tab.windowId, {
                focused: true
            }, function() {
                // focus/update tab
                ...
                if (params.tab.url == newURL) {
                    chrome.tabs.update(params.tab.id, {
                        active: true
                    }, function() {
                        resolve();
                    });
        ...
        if (that.id != null && !that.mustResync) {
            ...
            if (params.useGmailUI && that.mustResync) {
                ...
                if (that.resyncAttempts > 0) {
                    chrome.alarms.create(Alarms.RESYNC_ALARM_PREFIX + email, {
                        delayInMinutes: 1
                    });
                }
            }
    ...
    this.ensureMailAddressIdentified = async function() {
        if (!that.hasBeenIdentified()) {
            return fetchFeed({
                ensureMailAddressIdentifiedFlag: true
            });
        }
    }
    ...
    this.fetchGmailSettings = async function(params = {}) {
        ...
        if (lastFetchedGmailSettings && lastFetchedGmailSettings.diffInDays() > -FETCH_INTERVAL_IN_DAYS) { // !params.refresh && 
            ...
            if (data) {
                ...
                await that.saveSetting("gmailSettings", {
    ...
    async function fetchLabelsFromHtmlSource() {
        ...
        try {
            ...
            const data = await fetchText(that.getMailUrl({
                useStandardGmailUrl: true,
                urlParams: "ui=2&view=omni&rt=j&ik=" + idKey
            })); // "ui=2&view=up&rt=c&ik=" + idKey
            const labelStartStr = '[[[';
    ...
    this.remove = async function(oAuthForEmails, accounts) {
        if (accountAddingMethod == "autoDetect") {
            ...
            let tokenResponse = await oAuthForEmails.findTokenResponse({
                userEmail: that.getEmail()
            });
            if (tokenResponse) {
                removeCachedAuthToken(tokenResponse.access_token);
            }
            oAuthForEmails.removeTokenResponse({
                userEmail: that.getEmail()
            });
